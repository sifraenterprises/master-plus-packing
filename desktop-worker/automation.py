...
class PortalAutomationBase:
    ...
    async def start(self):
        ...
        profile_dir = Path(os.environ.get("TAFE_PROFILE_DIR", str(ROOT / "tafe-profile"))).expanduser()
    ...
    async def login(self):
        ...
        # A persistent profile may already have a valid TAFE session.
        logged_in = await self.page.locator(s["logged_in_indicator"]).count() > 0
        if logged_in:
            await self.log("Login Reused", "Existing TAFE portal session reused")
            return
...
class EwayPrintAutomation(PortalAutomationBase):
    """Read-only government E-Way Bill print flow with manual login."""
    module = "eway_print"

    async def start_session(self):
        s = self.selectors.get("eway_print", {})
        url = s.get("login_url", "https://ewaybillgst.gov.in/login.aspx")
        if self.is_test:
            await self.log("Portal Opened", "[TEST] E-Way Bill portal session simulated")
            return
        from playwright.async_api import async_playwright
        self.pw = await async_playwright().start()
        # Reuse a persistent Chromium profile so CAPTCHA/OTP login survives
        # across print jobs. A new login is requested only when the session
        # has actually expired.
        profile_dir = Path(os.environ.get("EWAY_PROFILE_DIR", str(ROOT / "eway-profile"))).expanduser()
        profile_dir.mkdir(parents=True, exist_ok=True)
        self.browser = await self.pw.chromium.launch_persistent_context(
        ...
        self.page = self.browser.pages[0] if self.browser.pages else await self.browser.new_page()
        self.page.set_default_timeout(30000)
        print_url = s.get("print_url", "")
        try:
            await self.page.goto(print_url or url, wait_until="domcontentloaded")
            logged_in = await self.page.locator(s.get("ewb_input", "input[type='text']")).count() > 0
        except Exception:
            logged_in = False
        if not logged_in:
            await self.page.goto(url, wait_until="domcontentloaded")
            await self.log("Manual Login Wait", "Complete E-Way Bill login/CAPTCHA/OTP in the visible browser, then press Enter here")
            await asyncio.to_thread(input, "After manual E-Way Bill login, press Enter to continue: ")

    async def print_one(self, ewb_number, download_dir):
        digits = re.sub(r"\D", "", str(ewb_number or ""))
        if len(digits) != 12:
            raise AutomationError("E-Way Bill number must contain exactly 12 digits")
        if self.is_test:
            path = Path(download_dir) / f"EWB_{digits}.pdf"
            _placeholder_pdf(path, digits)
            await self.log("PDF Downloaded", f"[TEST] Simulated {path.name}")
            return {"pdf_path": str(path), "pdf_name": path.name}
        s = self.selectors["eway_print"]
        await self.page.goto(s["print_url"], wait_until="domcontentloaded")
        configured = self.page.locator(s["ewb_input"])
        input_field = None
        for candidate in await configured.all():
            try:
                if await candidate.is_visible() and await candidate.is_editable():
        ...
        if input_field is None:
            ...
            editable = []
            for candidate in await fallback.all():
                try:
                    if await candidate.is_visible() and await candidate.is_editable():
            ...
            if len(editable) == 1:
                ...
                await self.log(
                    "Selector Fallback",
                    "Using the only visible editable input on the Print E-Way Bill page",
                )
            else:
                await self.capture_screenshot(f"eway_print_input_missing_{digits}")
                raise AutomationError(
                    "Print E-Way Bill field not identified: "
                    f"configured matches=0, visible editable inputs={len(editable)}"
                )
        await input_field.fill(digits)
        actual = re.sub(r"\D", "", await input_field.input_value())
        if actual != digits:
            raise AutomationError(
                f"E-Way Bill field verification failed: expected {digits}, got {actual or 'nothing'}"
            )
        await self.page.click(s["go_button"])
        await self.page.wait_for_selector(s["result_number"], timeout=30000)
        body_text = await self.page.locator("body").inner_text()
        if digits not in re.sub(r"\D", "", body_text):
            raise AutomationError(f"E-Way Bill mismatch: requested {digits} was not shown on the result page")
        path = Path(download_dir) / f"EWB_{digits}.pdf"
        await self.page.pdf(
            path=str(path),
            format="A4",
        ...
        await self.log("PDF Downloaded", f"E-Way Bill {digits} saved as {path.name}")
        return {"pdf_path": str(path), "pdf_name": path.name}

    async def return_home(self):
        """Return to the government portal home page between batch items."""
        home_candidates = self.page.locator(
            "a:has-text('Home'), button:has-text('Home'), "
            "input[type='button'][value*='Home' i], input[type='submit'][value*='Home' i]"
        )
        for candidate in await home_candidates.all():
            try:
                if await candidate.is_visible() and await candidate.is_enabled():
                    await candidate.click()
                    await self.page.wait_for_load_state("domcontentloaded")
                    await self.log("Navigation", "Returned to government portal Home")
                    return
            except Exception:
                continue
        await self.log("Navigation", "Home control not found; continuing through the Print EWB URL", level="WARN")


def _placeholder_pdf(path, digits):
    """Small valid PDF used only for TEST mode; no government portal is contacted."""
    body = f"BT /F1 16 Tf 72 760 Td (E-Way Bill TEST: {digits}) Tj ET"
...
class EWayBillAutomation(PortalAutomationBase):
    ...
    async def navigate_to_entry(self):
        ...
        if os.getenv("MANUAL_EWAY_ENTRY", "false").lower() in ("1", "true", "yes"):
            entry_link = self.page.locator(s["menu_eway_entry"])
            if await entry_link.count() > 0:
                await self.page.evaluate(
                    "(selector) => { setTimeout(() => document.querySelector(selector)?.click(), 0); }",
                    s["menu_eway_entry"],
                )
                ...
                await self.log(
                    "Navigation",
                    "Clicked E-Way Bill Entry twice using its stable portal link",
                )
                try:
                    await self.page.wait_for_selector(
                        s["eway_bill_no"], state="visible", timeout=15000
                    )
                    await self.log("Navigation", "Detected E-Way Bill Entry form")
                    return
                except Exception:
                    pass
            await self.log(
                "Navigation",
                "Manual handoff: open E-Way Bill Entry in TAFE and wait for the form, then press Enter",
            )
            await asyncio.to_thread(
                input,
                "Open E-Way Bill Entry in TAFE. When its form is visible, press Enter: ",
            )
            if await self.page.locator(s["eway_bill_no"]).count() > 0:
                await self.log("Navigation", "Detected E-Way Bill Entry form")
                return
            for frame in self.page.frames:
                if frame == self.page.main_frame:
                    continue
                if await frame.locator(s["eway_bill_no"]).count() > 0:
                    self.page = frame
                    await self.log("Navigation", "Detected E-Way Bill Entry form in portal frame")
                    return
            await self.capture_screenshot("eway_manual_navigation_missing")
            raise AutomationError(
                "Manual navigation finished, but the E-Way Bill form was not detected"
            )
        await self.page.evaluate(
            "(selector) => { setTimeout(() => document.querySelector(selector)?.click(), 0); }",
            s["menu_eway_entry"],
        ...
        try:
            await self.page.wait_for_selector(s["eway_bill_no"], state="visible", timeout=45000)
        except Exception:
            for frame in self.page.frames:
                if frame == self.page.main_frame:
                    continue
                try:
                    await frame.wait_for_selector(
                        s["eway_bill_no"], state="visible", timeout=5000
                    )
                    self.page = frame
                    break
                except Exception:
                ...
                await self.capture_screenshot("eway_navigation_selector_missing")
                raise AutomationError(
                    "E-Way Bill Entry tab opened, but its form did not load"
                )
    ...
    async def fill_form(self, data):
        ...
        eway_digits = re.sub(r"\D", "", str(data["eway_bill_number"]))
        if len(eway_digits) != 12:
            raise AutomationError(
                f"E-Way Bill must contain exactly 12 digits; received {len(eway_digits)}"
            )
        formatted_eway = " ".join(
            eway_digits[index:index + 4] for index in range(0, 12, 4)
        )
        await self.fill_and_verify(s["eway_bill_no"], formatted_eway)
        await self.log("Field Check", f"E-Way Bill formatted as {formatted_eway}")
...
class ASNAutomation(PortalAutomationBase):
    ...
    async def run_asn(self, data):
        ...
        for item in data["items"]:
            ...
            await link.click()
            invoice_row = self.page.locator("tr").filter(has_text=part).filter(
                has=self.page.locator("a").filter(has_text="Remove")
            ).first
            try:
                await invoice_row.wait_for(state="visible", timeout=15000)
            except Exception:
                await self.log(
                    "Parts Add Retry",
                    f"Part {part}: portal ignored the first Add-to-Invoice click; retrying once",
                    level="WARN",
                )
                await link.click()
                try:
                    ...
                    raise AutomationError(
                        f"Part {part} was not added to Invoice Material after two clicks"
                    )
            await self.log(
                "Parts Added",
                f"Part {part}: verified in Invoice Material",
            )
            if qty:
                qty_input = self.page.locator(s["invoice_row_qty"].replace("{part}", part)).first
                try:
                    await qty_input.wait_for(state="visible", timeout=5000)
                except Exception:
                    visible_qty_inputs = self.page.locator(
                        "input[id$=':txPartASNQty']:visible"
                    )
                    visible_count = await visible_qty_inputs.count()
                    if visible_count == 1:
                        qty_input = visible_qty_inputs.first
                    elif visible_count > 1:
                        raise AutomationError(
                            f"Could not safely identify ASN Qty for part {part}; "
                            f"{visible_count} visible candidate fields were found"
                        )
                    else:
                        qty_input = None
                if qty_input is not None:
                    qty_text = str(int(qty) if float(qty) == int(qty) else qty)
                    await qty_input.fill(qty_text)
                    await self.log("Parts Added", f"Part {part}: ASN Qty set to {qty_text}")
                else:
                    await self.log(
                        "Parts Added",
                        f"Part {part}: ASN Qty field is not active; continuing to active batch fields",
                        level="WARN",
                    )
            batches = await self.read_batches(part)
            if batches:
                if data.get("manual_batch_selection"):
                    if len(batches) == 1 and qty:
                        batch = batches[0]
                        container = self.page.locator(
                            s["batch_container"].replace("{part}", part)
                        ).first
                        batch_row = container.locator(
                            f"tr:has-text('{batch['batch_no']}')"
                        ).first
                        batch_qty_input = batch_row.locator(
                            s["batch_qty_input"]
                        ).first
                        await batch_qty_input.wait_for(
                            state="visible", timeout=30000
                        )
                        qty_text = str(
                            int(qty) if float(qty) == int(qty) else qty
                        )
                        await batch_qty_input.fill(qty_text)
                        await self.log(
                            "Batch Details",
                            f"Part {part}: Quantity To be Confirmed set to {qty_text}; "
                            "Batch Considerable Yes/No left for manual selection",
                        )
                    await self.log(
                        "Batch Details",
                        f"Part {part}: {len(batches)} batch(es) found; "
                        "Batch Considerable selection left manual",
                    )
                else:
        ...
        for attempt in (1, 2):
            try:
                ...
                try:
                    await upload_page.wait_for_selector(s["attach_success"], state="visible", timeout=10000)
                except Exception:
                    attached_name = os.path.basename(data["pdi_path"])
                    await upload_page.get_by_text(attached_name, exact=True).wait_for(
