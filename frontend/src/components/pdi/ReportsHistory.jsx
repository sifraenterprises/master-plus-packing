    ...
    <div className="space-y-4" data-testid="pdi-reports-history">
      ...
      <div className="border border-border rounded-sm overflow-x-auto bg-card">
        <Table>
          <TableHeader>
            <TableRow>
              {["Report No", "Date", "Part Name", "Item Code", "Invoice", "Customer", "Lot No", "Inspector", "Approver", "Rev", "Status", "Actions"].map((h) => (
          ...
          <TableBody>
            {data.items.map((rep) => (
              <TableRow key={rep.id} data-testid={`pdi-report-row-${rep.report_no}`}>
                ...
                <TableCell className="text-xs">{rep.inspector || "—"}</TableCell>
                <TableCell className="text-xs">{rep.approver || "—"}</TableCell>
                <TableCell className="text-xs font-mono" title={rep.source === "manual" ? `Uploaded by ${rep.created_by}` : `Generated by ${rep.created_by} · template revision ${rep.template_revision || 1}`}>{rep.source === "manual" ? "—" : `r${rep.template_revision || 1}`}</TableCell>
                <TableCell>
                  <Badge variant="outline" className={`rounded-sm text-[9px] uppercase ${rep.source === "manual" ? "border-sky-500/50 text-sky-500" : rep.status === "regenerated" ? "border-amber-500/50 text-amber-500" : "border-primary/40 text-primary"}`}>
                    {rep.source === "manual" ? "manual" : rep.status}{rep.regenerated_count ? ` ×${rep.regenerated_count}` : ""}
                  </Badge>
                </TableCell>
                <TableCell>
            ...
            {data.items.length === 0 && (
              <TableRow><TableCell colSpan={12} className="text-center text-xs text-muted-foreground py-8">No PDI reports yet. Generate one from the Generate tab.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
