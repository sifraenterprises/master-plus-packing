...
from routes.dqms_routes import router as dqms_router
...
api_router.include_router(dqms_router)
...
async def startup():
    ...
    await db.dqms_batches.create_index([("created_at", -1)])
    await db.dqms_batches.create_index("status")
    await db.dqms_batches.create_index("desktop_job_id")
